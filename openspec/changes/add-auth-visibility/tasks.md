...
- [x] Optional host-only OIDC: if `CONDUIT_OIDC_ISSUER` (identikey-core or any OP) is set, validate the OP token and mint a Conduit Biscuit. No AGPL dep.
...
Out of scope (bullets, not boxes): Recrypt; AGPL `identikey-oidc` OP crate; holder-bound §7 proofs; full cross-room restricted joins; opaque-token migration; `add-library-ops` / `add-in-process-client`.
