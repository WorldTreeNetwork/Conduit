...
Living truth for the two-crate Matrix kernel. Folded from
`add-embeddable-kernel` on 2026-09-10.

A `SHALL` in this file is evidence of intended current behavior. A
`SHALL` in `openspec/changes/` (including `archive/`) is not.

## Purpose

`conduit` is the tokio-hosted kernel. `conduit-server` is the axum
host. Embedding without a webserver is a crate-graph fact, not a
future HTTP SDK.
